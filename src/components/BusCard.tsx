...
import { BusRoute } from "@/data/buses";
import { stops } from "@/data/stops";
import { Clock, IndianRupee, ArrowRight, AlertTriangle, Zap, BadgeIndianRupee, Timer } from "lucide-react";
  ...
  return (
    <div className="animate-fade-in rounded-xl border border-border bg-card p-4 bus-card-shadow transition-shadow hover:bus-card-shadow-hover">
      ...
      <div className="flex items-start justify-between">
        <div>
          <h3 className="text-sm font-bold text-foreground">{bus.busNumber}</h3>
          <p className="text-xs text-muted-foreground">{bus.busName} · {busTypeLabel[bus.busType]}</p>
        </div>
        <div className="text-right">
          <p className="text-lg font-bold text-primary">₹{bus.price}</p>
          <span className={`inline-flex items-center gap-1 text-xs font-medium ${bus.type === "direct" ? "text-success" : "text-muted-foreground"}`}>
            {bus.type === "direct" ? t.results.direct : t.results.stops}
          </span>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center gap-3">
        <div className="flex flex-col items-center">
          ...
          <span className="text-[10px] text-muted-foreground">{getStopName(bus.fromId)}</span>
        </div>
        <div className="flex flex-1 flex-col items-center">
          <span className="text-[10px] text-muted-foreground">{formatDuration(bus.durationMinutes)}</span>
        ...
        <div className="flex flex-col items-center">
          ...
          <span className="text-[10px] text-muted-foreground">{getStopName(bus.toId)}</span>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center justify-between">
        ...
        {onSaveRoute && (
          <button
            onClick={onSaveRoute}
            className={`text-xs font-medium transition-colors ${isSaved ? "text-primary" : "text-muted-foreground hover:text-primary"}`}
          >
