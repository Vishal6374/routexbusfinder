...
const BusResults = ({ buses, onSaveRoute, savedRouteIds = new Set() }: BusResultsProps) => {
  ...
  const filtered = useMemo(() => {
    let result = [...buses];
    if (directOnly) result = result.filter((b) => b.type === "direct");
    return result;
  }, [buses, directOnly]);
  ...
  const sorted = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "departure":
      default:
        return arr.sort((a, b) => a.departure.localeCompare(b.departure));
    }
  }, [filtered, sortBy]);
  ...
  const cheapestId = useMemo(() => {
    ...
    return sorted.reduce((min, b) => (b.price < min.price ? b : min), sorted[0]).id;
  }, [sorted]);

  const fastestId = useMemo(() => {
    if (!sorted.length) return null;
    return sorted.reduce((min, b) => (b.durationMinutes < min.durationMinutes ? b : min), sorted[0]).id;
  }, [sorted]);
  ...
  const nextBusId = useMemo(() => {
    ...
    const upcoming = sorted.filter((b) => {
      const [h, m] = b.departure.split(":").map(Number);
    ...
    <div className="mt-6">
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-medium text-card-foreground transition-colors hover:bg-secondary"
        >
            ...
            <div className="flex items-center gap-2">
              ...
              {(["departure", "price", "duration"] as SortOption[]).map((opt) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${sortBy === opt ? "bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground hover:bg-muted"}`}
                >
      ...
      <div className="flex flex-col gap-3">
        {sorted.map((bus) => (
          <BusCard
            key={bus.id}
            bus={bus}
            highlight={getHighlight(bus.id)}
            onSaveRoute={onSaveRoute ? () => onSaveRoute(bus) : undefined}
            isSaved={savedRouteIds.has(bus.id)}
          />
        ))}
      </div>
    </div>
